...
import ThumbImgs from "../Thumbimgs";
...
class MatchGame extends Component {
  ...
  setTimer = () => {
    const { timer, isGameRunning } = this.state;
  ...
  render() {
    const { timer, score, initialImage, initailTab, isGameRunning } =
      this.state;
    const { imageUrl, id } = initialImage;
    const filterThumImgs = imagesList.filter(
      (eachThumb) => eachThumb.category === initailTab
    );
    return (
      <div className="container-fluid">
        <div className="match-game">
          ...
          {isGameRunning ? (
            <>
              <div className="row mx-auto mt-5 mb-5 mt-xl-3 mb-xl-3 img-card ">
                <img src={imageUrl} alt={id} className="img-fluid" />
              </div>
              <div className="row">
                  ...
                  {tabsList.map((eachTab) => {
                    return (
                      <TabItem
                        eachTab={eachTab}
                        key={eachTab.tabId}
                        isActiveTab={eachTab.tabId === initailTab}
            ...
            <div className="row">
              <div className="score-card d-flex flex-column align-items-center justify-content-center">
                <h1 className="text-light">Your Score</h1>
                <h3 className="text-light">{score}</h3>
